...
class MNIST(VisionDataset):
    ...
    def __init__(
        ...
        raw_data = self.data
        data_E_id = self.data.sqrt().round()
        data_S = torch.zeros_like(self.data)
        data_E = torch.zeros_like(self.data)
        sum_g = torch.zeros((self.data.shape[0]))
        sum_ez = torch.zeros((self.data.shape[0]))
        sum_E = torch.zeros((self.data.shape[0]))
        for r_idx in range(self.data.shape[1]):
            for c_idx in range(self.data.shape[2]):
                data_idx = (r_idx + 1) * self.data.shape[1] + (c_idx + 1)
                sum_g += 1.380648 * 1e-23 * torch.log(
                    torch.tensor(self.data.shape[1] * self.data.shape[2])
                ) - data_E_id[:, r_idx, c_idx] / (273 + 25)
                sum_ez += (
                    data_E_id[:, r_idx, c_idx]
                    * self.data.shape[1]
                    * self.data.shape[2]
                    * torch.exp(
                        -data_E_id[:, r_idx, c_idx] / (1.380648 * 1e-23 * (273 + 25))
                    )
                    / (273 + 25)
                )
                sum_E += self.data[:, r_idx, c_idx]

                data_S[:, r_idx, c_idx] = data_idx * (sum_g + sum_ez)
                data_E[:, r_idx, c_idx] = sum_E / data_idx
        self.data = torch.stack(
            [raw_data, data_E_id, data_S, data_E],
            dim=-1,
        )
